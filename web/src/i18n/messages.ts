...
const englishMessages = {
  ...
  'faq.a9':
    'A controlled castle or village no longer produces stock by itself: every territory you control yields `territory_income` R per action turn, plus `village_income` R if it carries a village, credited directly to your capital (see the next question on territory income). Mills are unrelated to that income and still work the same way: a mill is built on an empty controlled territory adjacent to a castle or village, or upgraded when it is adjacent to that source. It can reach level 3: construction costs `3 R`, then upgrades cost `5 R` and `7 R`. A `C M` at level 3 is rejected with no payment. Each level adds `+1 R` to **every** adjacent castle or village, without an owner filter. A level-1 mill between a village and two castles therefore adds `+1 R` to all three. An orphaned mill, with no adjacent castle or village, produces nothing. Existing mills above level 3 remain productive, but cannot be upgraded.',
  ...
  'faq.q11': 'How does a resource transfer (`T`) work?',
  'faq.a11':
  ...
  'faq.a14':
    "The command post checks each of your armies on its own: its own territory's production, plus the full stock and production of every controlled supply source it can reach, as if that army were the only one drawing on it. It does not simulate how several of your armies actually split a shared source's stock at resolution (see the questions on local rations and territory income above), so two armies that each look safe can still end up competing for the same source and see one of them starve once the turn resolves. It also always assumes a normal harvest: a bad harvest or bad weather card already drawn for this season, but not yet revealed, never changes this projection. It is never shown in winter, since ravitaillement does not happen then.",
  ...
  'online.notRequired': 'Nothing to submit',
...
const frenchMessages: Record<keyof typeof englishMessages, string> = {
  ...
  'faq.q11': 'Comment fonctionne un transfert de ressources (`T`) ?',
  'faq.a11':
  ...
  'faq.a14':
    'Le poste de commandement examine chacune de tes armées isolément : la production de sa propre case, plus le stock complet et la production de chaque source de ravitaillement contrôlée qu’elle peut atteindre, comme si cette armée était la seule à en dépendre. Il ne simule pas la façon dont plusieurs de tes armées se partagent réellement le stock d’une même source à la résolution (voir les questions sur les rations locales et le revenu territorial ci-dessus) : deux armées chacune non signalées peuvent donc quand même se disputer la même source et voir l’une d’elles affamée une fois le tour résolu. Il suppose aussi toujours une récolte normale : une carte de mauvaise récolte ou de mauvais temps déjà tirée pour cette saison mais pas encore révélée ne change jamais cette projection. Elle n’est jamais affichée en hiver, puisque le ravitaillement n’a pas lieu à cette saison.',
  ...
  'online.notRequired': 'Rien à soumettre',
