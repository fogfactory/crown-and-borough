...
2. MODÈLE SERVEUR (internal/server ou équivalent, SÉPARÉ du moteur) :
   - GameSession { ID, Name, Seed, State *models.GameState, Players
     []PlayerSlot { ID (P1, P2...), Name, Color }, Status (waiting |
   ...
   - POST /api/games/{id}/orders — { player, chains: [{noble, text}],
     ...
     pas résolu) :
       ...
       * si TOUS les joueurs vivants ont soumis → RÉSOLUTION immédiate
         ...
         jeu) → le rapport est stocké (Reports), l'état avance, les
         soumissions sont vidées pour le tour suivant ; réponse
         { submitted: true, resolved: true, report: <TurnReport> }
...
4. MODÈLE ASYNCHRONE (choix documentés) :
   ...
   - JOUEUR ÉLIMINÉ (choix documenté pour mise à jour du GDD — aucune
     règle de victoire n'existe encore) : un joueur est éliminé quand il
     ne contrôle aucun territoire, n'a plus aucune armée et plus aucun
     noble ; il est alors compté comme ayant soumis (soumission vide) et
     ne peut plus soumettre ; la partie est FINIE quand il ne reste qu'un
     seul joueur vivant (le gagnant)
