...
déconnecte les joueurs, mais les parties survivent — ils se réinscrivent et
retrouvent leurs parties). La migration Postgres est HORS périmètre.
...
3. RESTAURATION (au démarrage) :
   ...
   - Fichier corrompu (JSON invalide, version inconnue, état incohérent
     vérifié par les invariants de base : armées sur territoires existants,
...
5. TESTS (internal/server/persist_test.go) :
   ...
   - REDÉMARRAGE COMPLET : jouer 3 tours → restart → les 3 rapports sont
     ...
     navigateurs différents (P3.2) retrouvent la partie après réinscription
   - Invariant : les sessions meurent au restart (401) mais les parties
     survivent (re-join par le code)
