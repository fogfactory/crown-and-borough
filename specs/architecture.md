...
| `P` | Pillage | `P XXX` | XXX = case où l'armée se trouve : détruit l'infrastructure **la plus récente** de SA case + bonus R (balance.json) **crédité au lieu-dit contrôlé le plus proche** du joueur (perdu s'il n'en contrôle aucun) ; **aucune infrastructure → ordre invalide** | Chaîne brisée | Retente jusqu'à destruction |
