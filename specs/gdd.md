...
- **Entité Armée :** une armée est l'unique entité de force d'un territoire ; elle porte un **propriétaire** (`owner`) et une **taille** (`size`, nombre de troupes). Les ordres, le ravitaillement et les combats adressent l'armée par son territoire, jamais une troupe individuelle. Une armée est donc au plus une par case.
...
Un joueur peut programmer des séquences d'ordres successives (O1 → O2 → O3). **Chaque transition (chaque ordre) possède sa propre liaison** : unique ou boucle. **Il n'existe pas de modification de chaîne** : une armée qui reçoit une chaîne remplace la précédente. Une chaîne est portée par une **armée**, adressée par le territoire, et s'applique à toute sa taille — jamais d'ordres mixtes. La **jonction (J)** ne peut figurer qu'en **dernier ordre** d'une chaîne.
...
- **Chevauchée :** un noble se déplace AVEC l'armée de sa case : quand une armée quitte sa case (A, J, ou retraite), le noble la suit — l'armée se déplace toujours en bloc, la dispersion D est la seule façon de la séparer. En cas de **dispersion (D)**, une **astérisque** sur une destination désigne les nobles qui s'y rendent :
  ...
  `*` seul = tous les nobles restants ; `*CODE` = le noble désigné (trigramme du prénom) ; chaque noble au plus une fois. **Si des nobles chevauchent l'armée et que l'ordre ne les mentionne pas TOUS** (aucune astérisque, ou nobles non couverts) → l'ordre est **INVALIDE** (aucune destination par défaut).
- **Jamais seul :** un noble ne peut pas être seul : toute destination désignée porte au moins une troupe, et un noble ne peut être recruté que sur une case contenant une armée du joueur. (Cas limite toléré : armée détruite sur une case restée vide, le noble y demeure seul.)
