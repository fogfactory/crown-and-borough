...
function chainPlaceholder(noble: Noble): string {
  return `${noble.code} # ${noble.name}\nXXX A YYY`
}
      ...
      <section className="space-y-3 border-t border-[#b7a786]/50 pt-5">
        <div>
          ...
          <p className="mt-1 text-xs leading-relaxed text-[#806f57]">
            Une ligne par investissement. Les ordres sont appliqués dans l&apos;ordre saisi.
          </p>
        </div>
        <textarea
          value={winterDraft}
  ...
  return (
    <section className="space-y-3 border-t border-[#b7a786]/50 pt-5">
      <div>
        ...
        <p className="mt-1 text-xs leading-relaxed text-[#806f57]">
          Une chaîne complète par noble. Une zone vide signifie que le noble n&apos;émet pas ce tour.
        </p>
      </div>
      {nobles.length === 0 ? (
        ...
        nobles.map((noble) => (
          <label key={noble.code} className="block space-y-1.5">
            ...
            <textarea
              ...
              placeholder={chainPlaceholder(noble)}
