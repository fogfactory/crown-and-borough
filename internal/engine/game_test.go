...
// TestGenerateMapRecoversFromUnluckySeeds pins UUID v4 seeds, as generated by
// the store for games created without a seed, whose first draw violates a
// structural constraint: an unbalanceable region partition and an unmeetable
// degree cap. Generation must still succeed, deterministically.
func TestGenerateMapRecoversFromUnluckySeeds(t *testing.T) {
	assets := loadGameTestAssets(t)
	for _, seed := range []string{
		"3cef09ff-14be-4392-a801-f6eaee414091", // cannot balance regions within 4 territories
		"585aebd6-48c8-48ae-b378-39fcd48ca6d1", // cannot enforce max degree for territory site-3
	} {
		t.Run(seed, func(t *testing.T) {
			const players = 2
			first, err := GenerateMap(seed, players, assets)
			if err != nil {
				t.Fatalf("GenerateMap(%q) = %v", seed, err)
			}
			second, err := GenerateMap(seed, players, assets)
			if err != nil || !reflect.DeepEqual(first, second) {
				t.Fatalf("GenerateMap(%q) is not deterministic (err=%v)", seed, err)
			}
			if err := mapgen.ValidateRegions(first.Territories, first.Regions); err != nil {
				t.Fatalf("ValidateRegions = %v", err)
			}
			minimum, maximum := len(first.Regions[0].Territories), len(first.Regions[0].Territories)
			for _, region := range first.Regions[1:] {
				minimum = min(minimum, len(region.Territories))
				maximum = max(maximum, len(region.Territories))
			}
			if limit := 2 * players; maximum-minimum > limit {
				t.Fatalf("region sizes range from %d to %d, exceeding bound %d", minimum, maximum, limit)
			}
		})
	}
}
