...
8. Immediately before submitting, perform a final diplomatic sweep. This is
   mandatory because another player may have sent a counter-offer while you
   were composing orders:

   ```bash
   ./scripts/chat-poll.sh --all-dms
   ```

   Re-evaluate any message that changes a pact, threat, or target. Do not treat
   an earlier decision as final merely because your draft is already written.

9. Confirm `status: pending` or `status: resolved`. On rejection, read the
   server error, repair the order, and resubmit; do not silently switch to a
   different plan.
10. Send only the negotiations and public message justified by the result.
11. Wait for the state revision to advance. Do not submit twice just because a
...
The first line is a permanent identity anchor generated by `persona-init.sh`:
it names the display name and the resolved `play_style`, `trust`, and `tone`.
Do not delete, move, or soften that sentence when updating the rest of the
memory; reread it before making a strategic decision.
