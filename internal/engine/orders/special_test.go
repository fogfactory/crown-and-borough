...
func TestParseDeckOrdersAliasesAndComments(t *testing.T) {
	...
	if len(parsed) != 4 {
		t.Fatalf("parsed = %#v, want 4 orders", parsed)
	}
	if parsed[0].Type != models.DeckOrderTypeDiscard || parsed[0].Kind != models.CardKindFairWeather {
		t.Errorf("discard = %#v", parsed[0])
	}
	if parsed[1].Kind != models.CardKindAbundantHarvest {
		t.Errorf("aliases = %#v", parsed)
	}
	if parsed[2].Type != models.DeckOrderTypePlay || parsed[2].Kind != models.CardKindFairWeather || parsed[2].RegionSeed != "ROS" {
		t.Errorf("play = %#v", parsed[2])
	}
}
...
func TestParseDeckOrdersRejectsInvalidShapesKindsAndSeeds(t *testing.T) {
	...
	if parsed, parseErrors := ParseDeckOrders("T C", game); parsed != nil || len(parseErrors) != 1 {
		t.Fatalf("parsed T C = %#v/%#v, want one error", parsed, parseErrors)
	}
}
...
func TestParseDeckOrdersPreservesLineNumbersAndAtomicity(t *testing.T) {
	...
	parsed, parseErrors := ParseDeckOrders("\n# comment\nT C\nP BT BAD\n", game)
	if parsed != nil || len(parseErrors) != 2 || parseErrors[0].Line != 3 || parseErrors[1].Line != 4 {
