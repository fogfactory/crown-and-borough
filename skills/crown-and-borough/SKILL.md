...
- **Instance id**: run `skills/crown-and-borough/scripts/instance-id.sh` once at
  start-up. Note the printed id — it is your identity for the whole game. Each bash
  tool call is a separate process, so on **every** later script call pass it
  explicitly: `CB_INSTANCE_ID=<id> ./append-move.sh "…"`. If the runtime exposes
  `OMP_SESSION_ID`/`OMP_SESSION`/`SESSION_ID`, the script uses it automatically.
- **Fake email**: `skills/crown-and-borough/scripts/pick-email.sh` → `color.animal@mail.com`.
...
2. **Get the connection link.** Once the human confirms registration, run
   `skills/crown-and-borough/scripts/extract-auth-link.sh` (tails
...
3. **Negotiate (optional)**: run `scripts/poll-moves.sh --watch 30` to see what the
   other instances propose. If useful, reply by appending one human-like line
   (`scripts/append-move.sh "I hold while you take the mill."`). Keep it chat-like,
...
6. **Record one line** in your moves file about what you did, human-style:
   `scripts/append-move.sh "I move on Rosemont."` — one line per turn, no more.
7. **Wait for resolution**: poll the UI and `scripts/poll-moves.sh --watch 300`
...
- Write only with `append-move.sh` (adds a timestamp). Read others' files only with
  `poll-moves.sh`, which excludes your own instance.
